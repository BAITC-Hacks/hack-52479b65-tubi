...
## Для жюри: финальная версия решения

**Для проверки используйте ветку [feature/business-ui](https://github.com/BAITC-Hacks/hack-52479b65-tubi/tree/feature/business-ui).** В ней собраны backend, интерфейс RU/KK, каталог, отклики и отзывы о работе команд.

- Проверенный коммит приложения: [9c4039b](https://github.com/BAITC-Hacks/hack-52479b65-tubi/commit/9c4039b85dc2e63dcea0bd3dc1bf4f4341e0a8ff).
- Результат проверки: **58 frontend-тестов, 69 backend-тестов и production-сборка прошли**.
- [Инструкция установки и запуска](https://github.com/BAITC-Hacks/hack-52479b65-tubi/blob/feature/business-ui/README.md#установка-и-запуск) · [Сценарий демонстрации](https://github.com/BAITC-Hacks/hack-52479b65-tubi/blob/feature/business-ui/README.md#пример-проверки-и-демо-за-5-минут).

```sh
git clone --branch feature/business-ui https://github.com/BAITC-Hacks/hack-52479b65-tubi.git
```

Публичный адрес развёртывания не предоставлен; приложение запускается локально. API-ключ не включён в репозиторий: он задаётся в локальном `server/.env`. Без ключа доступен явно обозначенный деморежим.
...
[Что делает проект](#что-делает-проект) · [Что реализовано](#что-реализовано) · [Технологии и архитектура](#технологии-и-архитектура) · [Установка и запуск](#установка-и-запуск) · [Пример проверки](#пример-проверки-и-демо-за-5-минут) · [Данные и сервисы](#используемые-данные-и-внешние-сервисы) · [Ограничения](#известные-ограничения) · [Развёрнутая версия](#развёрнутая-версия)
...
Для Казахстана первый сценарий — небольшие задачи местного бизнеса для студенческих команд: анализ отзывов кофейни в Алматы, обработка заявок, автоматизация отчётности. Конструктор, каталог, карточки, отклики и отзывы поддерживают русский и казахский языки. Backend возвращает вопросы и системные сообщения на выбранном языке. Пользовательский текст автоматически не переводится. Это гипотеза применения, которую предстоит проверить на пилоте с бизнесом и университетом.

**Основной путь:** описание → минимум 3 уточняющих вопроса → редактируемая карточка → рейтинг 0–100 → подтверждение → публикация → отклик команды → ручной выбор бизнеса.

## Что реализовано

| Требование кейса | Реализация | Где проверить |
| --- | --- | --- |
| Описание и минимум 3 уточнения | Вопросы привязаны к полям карточки; уже известные сведения учитываются | «Создать задачу» → «Перейти к вопросам» |
...
| Редактирование и подтверждение | Все поля доступны для правки; изменение снимает подтверждение, ответы сохраняются при ошибках и возврате | Шаги «Карточка» → «Проверка» → «Публикация» |
...
| Отзыв о работе команды | После подтверждения этапа бизнес оставляет оценку 1–5 и комментарий; отзыв виден обеим сторонам | Блок «Отзыв о работе команды» под откликом |
| Сохранность данных | SQLite, транзакции, демоданные только при первом заполнении пустой базы | Перезапуск и backend-тесты |
| RU/KK для конструктора | Выбор языка сохраняется; интерфейс, подсказки и новые запросы используют его без потери введённого текста | Переключатель «Қазақша / Русский» |
| Прямые ссылки на задачи | Карточка открывается по `?task=ID` и восстанавливается при перезагрузке; ссылка копируется кнопкой | «Скопировать ссылку на задачу» |
...
**Публичная ссылка на развёрнутое приложение пока не указана.** В репозитории не настроен GitHub Pages; ссылка на GitHub ведёт к исходному коду. Для проверки жюри предусмотрен локальный запуск по инструкции ниже: [http://127.0.0.1:8001/](http://127.0.0.1:8001/). Этот адрес работает только на компьютере с запущенным сервером.

Проверенная совместная версия находится в ветке [feature/business-ui](https://github.com/BAITC-Hacks/hack-52479b65-tubi/tree/feature/business-ui): в неё включён готовый backend из `feature/backend-quality`. В `main` эти изменения автоматически не объединялись.
...
Скачайте проверенную совместную версию:
...
Заголовок `Accept-Language: ru` или `Accept-Language: kk` выбирает язык вопросов, предупреждений, ошибок и подсказок рейтинга. По умолчанию используется `ru`; поддерживаются региональные теги и списки с `q`. Общий HTTP-клиент отправляет выбранный язык автоматически. Уже полученные вопросы и предупреждения сохраняют язык исходного ответа; интерфейс объясняет это при переключении. Пользовательское содержание остаётся на исходном языке.
...
| --- | --- | --- |
| Интерфейс | React 19, TypeScript, Vite 7, CSS, Lucide React | Пошаговая форма, каталог, отклики, иконки и production-сборка |
| API | Python 3.11+, FastAPI, Uvicorn, Pydantic 2 | HTTP-маршруты, валидация запросов/ответов и запуск сервера |
| Хранение | SQLite через стандартный `sqlite3` | Карточки, команды, отклики и баллы без отдельного сервера БД |
| AI | OpenAI SDK, Responses API; по умолчанию `gpt-4o-mini` | Уточняющие вопросы и сборка карточки с проверкой цитат |
| Настройки | `python-dotenv` и переменные окружения | Локальный ключ, выбор модели, путь к базе |
| Проверки | unittest, FastAPI TestClient, Vitest, React Testing Library, jsdom | Backend, состояние форм, локализация и интеграция экранов |

Точные версии зависимостей закреплены в [server/requirements.txt](server/requirements.txt), [server/requirements-dev.txt](server/requirements-dev.txt) и [web/package-lock.json](web/package-lock.json).
...
| Модуль | Назначение |
| --- | --- |
| `web/src/features/business/` | Конструктор задачи, состояние формы, рейтинг и словари RU/KK |
| `web/src/features/marketplace/` | Каталог, карточка задачи, отклики и действия бизнеса |
| `web/src/shared/`, `web/src/App.tsx` | HTTP-клиент, локализация, общие стили и навигация |
...
| Источник | Что используется | Как обрабатывается |
| --- | --- | --- |
| [server/data/seed.json](server/data/seed.json) | Вымышленные задачи, команды и отклики для демонстрации | Загружаются только в пустую базу; это не реальные заказы и не результаты пилота |
| Пользовательский ввод | Описание, ответы, поля карточки, контакт, идея и план команды | Подтверждённые карточки, отклики и решения сохраняются в локальной SQLite |
| [contracts/fixtures.json](contracts/fixtures.json) | Примеры запросов и ответов API | Для документации и параллельной разработки, не внешний датасет |
| OpenAI API | Описание задачи и ответы, необходимые для генерации | Передаются внешнему провайдеру при настроенном AI; секретный ключ остаётся на сервере |

Внешние датасеты, скрейпинг, обучение модели и векторный поиск не используются. Упоминание CSV в демонстрационной карточке описывает предполагаемые данные задачи: MVP не загружает и не анализирует сам файл отзывов.

OpenAI — внешний сервис генерации. При отсутствии ключа или ошибке запрос заменяется явно обозначенными локальными правилами. `store=False` передаётся в запросе, но не означает гарантию отсутствия любой обработки или хранения у провайдера. Для демонстрации используйте вымышленные или обезличенные сведения.

При установке пакеты скачиваются из npm и PyPI. После сборки интерфейс и иконки раздаются локальным сервером; шрифты системные. Для работы в деморежиме интернет не требуется. Файл `.env` и рабочая SQLite исключены из Git.
...
После подтверждения выполненного этапа бизнес может оставить отзыв: целую оценку от 1 до 5 и непустой комментарий до 2000 символов. Отзыв привязан к отклику и сохраняется в SQLite. `POST /api/proposals/{id}/review` возвращает обновлённый отклик с полем `review`; у старых откликов оно равно `null`. Повтор того же отзыва не создаёт дубликат, попытка заменить опубликованный отзыв возвращает `409`. Отзыв не меняет рейтинг готовности задачи и баллы команды.
...
Проверено для совместной версии: **58 frontend-тестов и 69 backend-тестов прошли**, `npm run build` завершился успешно. В браузере пройден сценарий от описания до ручного выбора команды на отдельной SQLite в деморежиме; дополнительно проверены казахский каталог и открытие карточки по прямой ссылке после перезагрузки. Настоящий OpenAI в рамках этой проверки не вызывался.
...
Проверяются сохранение ответов при ошибке, запрет продолжения с пустым описанием, сброс подтверждения после правки, защита от устаревшего расчёта рейтинга, RU/KK без потери формы и повтор публикации с прежним ID задачи.

Для отзывов проверяются доступность только после подтверждённого этапа, границы оценки, пустые комментарии, повторные запросы, сохранение после перезапуска и сохранение ввода при ошибке API. Ошибка обновления списка после успешного сохранения не удаляет отзыв из интерфейса.
...
## Пример проверки и демо за 5 минут
...
| 0:35–1:25 | «Создать задачу» → описание → «Перейти к вопросам» | Показать минимум три вопроса о недостающих сведениях. Если включён fallback, назвать его деморежимом. |
...
| 2:35–3:05 | «Проверить рейтинг и подтвердить» → флажок → «Подтвердить и сохранить» → «Опубликовать задачу» | Решение о публикации принимает человек; задача появляется в каталоге. |
...
Подтвердите и сохраните карточку, затем опубликуйте её отдельной кнопкой. От имени команды отправьте идею «Сгруппируем отзывы и покажем основные темы», план «Изучим CSV, согласуем темы, подготовим таблицу», срок «7 дней после получения данных». До решения бизнеса отклик должен иметь статус «На рассмотрении». Вернитесь в роль бизнеса, нажмите «Выбрать команду» и проверьте статус «Команда выбрана».

**Проверка отзыва:** на демонстрационном примере выполненной работы нажмите «Подтвердить выполненный этап», выберите оценку и напишите «Команда подготовила понятный прототип и учла обратную связь». Нажмите «Опубликовать отзыв». Оценка и текст должны остаться под откликом после перезагрузки и быть видимыми в роли команды. До подтверждения этапа форма отзыва недоступна.
...
SQLite подходит текущему сценарию демонстрации; нагрузочные испытания не проводились. Интерфейс использует RU/KK и общую тёплую палитру. Введённая форма сохраняется при переходах между шагами и ошибках, но несохранённый черновик может потеряться при перезагрузке страницы или уходе из конструктора. Во время AI-запроса и сохранения навигация внутри приложения заблокирована. Прямая ссылка на локальном адресе работает только на компьютере, где запущен сервер.

Рейтинг измеряет заполненность, а цитаты подтверждают присутствие фрагмента во входе — экспертная проверка содержания остаётся за человеком. Настоящий AI зависит от ключа, доступности модели, сети и API-бюджета. Fallback не равнозначен работе генеративной модели.

Если ответ успешного создания карточки потеряется в сети, повтор создания может дать дубль: ключ идемпотентности создания пока не реализован. Повтор публикации уже сохранённой карточки использует её существующий ID.

Отзывы пока одноразовые, без редактирования, модерации и общего рейтинга команды. Проверка бизнес-роли в интерфейсе не заменяет серверную авторизацию: в этом MVP доступ к API общий. Сервер проверяет принятие отклика и подтверждение этапа перед отзывом.
