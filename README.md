...
Для Казахстана первый сценарий — небольшие задачи местного бизнеса для студенческих команд: анализ отзывов кофейни в Алматы, обработка заявок, автоматизация отчётности. Конструктор задачи и общая навигация поддерживают русский и казахский языки; каталог и отклики пока на русском. Backend возвращает вопросы и системные сообщения на выбранном языке. Пользовательский текст автоматически не переводится. Это гипотеза применения, которую предстоит проверить на пилоте с бизнесом и университетом.
...
Проверено для совместной версии: **47 frontend-тестов и 69 backend-тестов прошли**, `npm run build` завершился успешно. В браузере пройден сценарий от описания до ручного выбора команды на отдельной SQLite в деморежиме; ошибок JavaScript не обнаружено. Настоящий OpenAI в рамках этой проверки не вызывался.
...
SQLite подходит текущему сценарию демонстрации; нагрузочные испытания не проводились. Конструктор и оболочка переведены на RU/KK, каталог и отклики пока на русском и используют часть прежних стилей. Введённая форма сохраняется при переходах между шагами и ошибках, но несохранённый черновик может потеряться при перезагрузке страницы или уходе из конструктора.
