...
describe("shared HTTP and locale", () => {
  it.each([["ru", "ru"], ["kk", "kk"], ["en", "ru"]] as const)("sends %s as API language %s and keeps the body unchanged", async (locale, apiLocale) => {
    setLocale(locale);
  ...
  it("localizes network and validation errors without dropping metadata", async () => {
    ...
    expect(errorMessage(error as ApiError, "en")).toBe("Check your input and try again.");
